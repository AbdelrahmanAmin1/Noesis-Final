...
| ffmpeg    | Video assembly                   | Bundled via `@ffmpeg-installer/ffmpeg`; optional system install: https://ffmpeg.org/download.html |
...
| Windows SAPI / espeak-ng *(fallback)* | Backup TTS if Piper isn't installed | Windows SAPI is built in; `apt install espeak-ng` / `brew install espeak` |
...
cd backend
npm install
cp .env.example .env          # then edit JWT_SECRET and TTS_VOICE_PATH if needed
...
- `GET  /api/materials`
- `POST /api/materials` — multipart upload, field `file` (PDF/DOCX/DOC/TXT/MD/PPTX/PPT ≤ 25 MB) → `{ material_id, job_id }` (202 Accepted)
...
2. **Audio** — Per-slide narration → `tts.service`. Engines tried in order based on `TTS_ENGINE`: Piper → Windows SAPI → espeak-ng → `say` (macOS). Silent audio is only allowed when explicitly using `TTS_ENGINE=silence` or `NOESIS_ALLOW_SILENT_TTS=true`.
3. **Slides** — `node-canvas` renders 1280×720 PNGs when available. If `canvas` isn't installed, ffmpeg `drawtext` renders readable Noesis-style slide frames instead of blank placeholders.
4. **Combine** — Per slide: ffmpeg encodes `image + audio` → `seg_N.mp4`. All segments are concatenated and re-encoded to H.264/AAC → `uploads/videos/<id>.mp4`, then verified with ffprobe for video and audio streams.

Artifacts are persisted between steps; failed jobs surface via `/api/jobs/:id`.

Noesis uses the bundled `@ffmpeg-installer/ffmpeg` and `@ffprobe-installer/ffprobe` binaries by default. If you need to override them, set `FFMPEG_PATH` and `FFPROBE_PATH` in `.env` to full executable paths.

---

## 5. Troubleshooting

- **`ffmpeg_not_found` / `spawn ffmpeg ENOENT`** - run `npm install` so the bundled static ffmpeg package is present. If you prefer a system ffmpeg, set `FFMPEG_PATH` in `.env` to the full executable path, for example `C:/ffmpeg/bin/ffmpeg.exe`.
- **`ffmpeg_*` encode errors** - ffmpeg was found but failed during encoding. Check `/api/jobs/:id` for the final stderr snippet.

- **`ollama_404 / model not found`** — run `ollama pull llama3.2:3b` and `ollama pull nomic-embed-text`.
- **`tts_unavailable`** — install Piper (recommended), use Windows SAPI with `TTS_ENGINE=sapi`, or install `espeak-ng`. Set `TTS_VOICE_PATH` to a downloaded Piper voice file when using Piper.
- **PowerPoint uploads** — `.pptx` decks are indexed by extracting slide text and speaker-note text from the Office XML package. Legacy `.ppt` uploads are accepted by validation but ingestion fails with a clear message asking you to save as `.pptx`.
- **Slide text does not render** — ffmpeg `drawtext` could not find fonts. Install a system font package or install/build the optional `canvas` dependency.
